...
const kNonDef = 1 - 0.02 - 9 * 0.01 - 0.3;
...
const listHangIsoNodesNonDef = (powers) => {
    const hangNodes = [];
    const isoNodes = [];

    for (let nodeId in powers) {
        if (powers[nodeId] === 1) {
            hangNodes.push(nodeId);
        } else if (powers[nodeId] === 0) {
            isoNodes.push(nodeId);
        }
    }
...
const checkSimilarityNonDef = (powers) => {
...
if (contextNonDef) {
    ...
    console.log("The list of hanged and isolated nodes in non-defined graph:", listHangIsoNodesNonDef(powers));

    const similarity = checkSimilarityNonDef(powers);
    if (similarity === false) {
        console.log('Is the non-defined graph similar?', similarity);
    } else {
        console.log('The similarity power of non-defined graph is:', similarity);
    }
